...
export default function Home() {
    ...
    const browse = (value = 'All pieces') => { setCategory(value); setShowAll(true); setPanel(null); document.getElementById('collection')?.scrollIntoView({ behavior: 'smooth' }); };
    const toggleSaved = (id: string) => { setSaved(previous => previous.includes(id) ? previous.filter(x => x !== id) : [...previous, id]); };
    const addToBag = useCallback((p: Product, chosen: string, count: number) => {
        setCart(previous => { const exists = previous.find(line => line.id === p.id && line.finish === chosen); return exists ? previous.map(line => line === exists ? { ...line, quantity: Math.min(10, line.quantity + count) } : line) : [...previous, { id: p.id, finish: chosen, quantity: count }]; });
    ...
    const filtered = useMemo(() => { const list = products.filter(p => category === 'All pieces' || p.category === category); if (sort === 'price-low')
        list.sort((a, b) => a.price - b.price); if (sort === 'price-high')
        list.sort((a, b) => b.price - a.price); if (sort === 'name')
        list.sort((a, b) => a.name.localeCompare(b.name)); return list; }, [category, sort]);
    const displayed = showAll || category !== 'All pieces' || sort !== 'featured' ? filtered : filtered.slice(0, 4);
    const changeLine = (index: number, value: number) => setCart(previous => previous.map((line, i) => i === index ? { ...line, quantity: value } : line));
    const productCard = (p: Product) => <article className="product-card" key={p.id}><div className="product-image-wrap"><button className="product-image-button" onClick={() => openProduct(p)} aria-label={`View ${p.name}`}><img src={p.image} alt={p.name} loading="lazy" style={{ objectPosition: p.position }}/></button>{p.badge && <span className="product-label">{p.badge}</span>}<button className={`save-button ${saved.includes(p.id) ? 'is-saved' : ''}`} aria-label={`${saved.includes(p.id) ? 'Unsave' : 'Save'} ${p.name}`} aria-pressed={saved.includes(p.id)} onClick={() => toggleSaved(p.id)}><Heart size={18}/></button><button className="quick-add" aria-label={`Choose options for ${p.name}`} onClick={() => openProduct(p)}><Plus size={20}/></button></div><div className="product-title"><h3><button onClick={() => openProduct(p)}>{p.name}</button></h3><span>{money(p.price)}</span></div><p className="product-detail">{p.material} · {p.finishes[0].name}</p><div className="swatches" aria-label={`Available in ${p.finishes.length} finish${p.finishes.length === 1 ? '' : 'es'}`}>{p.finishes.map(f => <button key={f.name} style={{ background: f.color }} aria-label={`View ${p.name} in ${f.name}`} onClick={() => { openProduct(p); setFinish(f.name); }}/>)}</div></article>;
    return <>
    <Toaster theme="light" position="bottom-left"/>
    <a className="skip-link" href="#collection">Skip to collection</a>
    <div className="announcement">A little more considered. A little more you. <a href="#collection">Discover the collection <ArrowUpRight size={12}/></a></div>
    <header className="site-header">
      <a href="#" className="wordmark" aria-label="Forma home">forma<span>®</span></a>
      <nav className="desktop-nav" aria-label="Main navigation"><a href="#collection" onClick={() => browse()}>Furniture</a><a href="#collection" onClick={() => browse('Lighting')}>Lighting</a><a href="#collection" onClick={() => browse('Objects')}>Objects</a><a href="#story">Our story</a></nav>
      <div className="header-actions"><button aria-label="Search furniture" onClick={() => setSearchOpen(true)}><Search /></button><button aria-label={`Saved pieces (${saved.length})`} onClick={() => setPanel('saved')} className="saved-nav"><Heart />{saved.length > 0 && <i>{saved.length}</i>}</button><button className="bag-button" aria-label={`Open shopping bag (${count} items)`} onClick={() => setPanel('bag')}><ShoppingBag /><span>Bag ({count})</span>{count > 0 && <i className="mobile-bag-count">{count}</i>}</button><button className="mobile-menu" aria-label="Open menu" onClick={() => setPanel('menu')}><Menu /></button></div>
    </header>
    <main>
      <section className="hero">
        ...
        <div className="hero-content"><p className="eyebrow">THE ART OF FEELING AT HOME</p><h1>Good design.<br /><em>Better living.</em></h1><p>Considered furniture. Natural materials.<br />For the everyday moments that matter.</p><a className="button button-light" href="#collection">Explore the collection <ArrowUpRight size={19}/></a></div>
      ...
      <section className="collection section-wrap" id="collection">
        <div className="section-heading"><div><p className="eyebrow">FIND YOUR EVERYDAY FAVOURITES</p><h2>A place for <em>good things.</em></h2></div><button className="text-link" onClick={() => browse()}>Shop all furniture <ArrowUpRight size={18}/></button></div>
        <div className="collection-controls"><div className="categories" aria-label="Filter products by category">{categories.map(c => <button key={c} className={category === c ? 'active' : ''} aria-pressed={category === c} onClick={() => { setCategory(c); setShowAll(true); }}>{c}</button>)}</div><Select value={sort} onValueChange={setSort}><SelectTrigger aria-label="Sort products" className="sort-select"><SlidersHorizontal size={14}/><SelectValue /></SelectTrigger><SelectContent><SelectItem value="featured">Featured</SelectItem><SelectItem value="price-low">Price: low to high</SelectItem><SelectItem value="price-high">Price: high to low</SelectItem><SelectItem value="name">Name: A to Z</SelectItem></SelectContent></Select></div>
        <div className="product-grid" id="products" aria-live="polite">{displayed.map(productCard)}</div>
        {!showAll && category === 'All pieces' && sort === 'featured' && <div className="view-collection"><button className="button button-outline" onClick={() => setShowAll(true)}>View all {products.length} pieces <ArrowRight size={17}/></button></div>}
      </section>
      <section className="story section-wrap" id="story"><p className="eyebrow">LESS, BUT WITH MEANING.</p><h2>A home is a collection<br />of things you <em>love.</em></h2><p>We believe the pieces you live with should make you feel something. A beautiful curve. The warmth of wood. A quiet corner that’s entirely yours.</p><a className="text-link" href="#collection">Find your piece <ArrowRight size={18}/></a></section>
    </main>
    <footer className="site-footer"><div className="footer-brand"><a href="#" className="wordmark">forma<span>®</span></a><p>Considered pieces. Everyday living.</p></div><nav aria-label="Footer navigation"><button onClick={() => setInfo('Delivery & returns')}>Delivery & returns</button><button onClick={() => setInfo('Care guide')}>Care guide</button><button onClick={() => setInfo('About this store')}>About this store</button></nav><div className="footer-bottom"><span>© {new Date().getFullYear()} Forma Studio</span><span>Concept store · USD · Demo checkout</span></div></footer>
